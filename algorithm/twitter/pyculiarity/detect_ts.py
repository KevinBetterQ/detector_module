...
from algorithm.twitter.pyculiarity.date_utils import format_timestamp, get_gran, date_format, datetimes_from_ts,transforTime
from algorithm.twitter.pyculiarity.detect_anoms import detect_anoms
...
def detect_ts(df, max_anoms=0.10, direction='pos',
    ...
    if not isinstance(df, DataFrame):
        ...
        # 备用代码
        '''
        # 如果不是datetime 格式 从时间冲转化为datatime
        if (not (df.dtypes[0].type is np.datetime64) and (df.dtypes[0].type is np.int64)):
            df = transforTime(df)
            df = format_timestamp(df)
        '''
    ...
    if longterm:
        ...
        all_data = []
        # range（1,3,2）即：1,2。第三个数字2是代表步长。如果不设置，就是默认步长为1  第三个参数是步长 前两个参数是取值 前闭后开
        # 无论是 sec min day 都是以2周作为步长
